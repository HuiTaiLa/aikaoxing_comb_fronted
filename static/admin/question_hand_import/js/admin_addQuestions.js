...
$(document).ready( function() {
	...
	$(".combBatchPanel input[name=insert_type]").change(function(e) {
		...
		$('.batch-preview-box .toolbar .title').text('检查区：');
	...
	$("#importBtn").click(function(e) {
		...
		if(text==""){
			...
			$.ajax({
              ...
              url: "/baseinfo/admin/edit_mix_question/?dataType=addBatch",
			  data: dataForm,
	...
	$("#saveAndAddBtn").click(function(e) {
		...
		if(type==6){
			...
			if(classification == "1"||classification == ""){
				alert("请选择试题分类！");
				return false;
			}
			saveComb();
...
function serializeForm(){
  ...
  // $("#asyncForm div").append('<textarea name="rawQuestion"></textarea>');
  // $("#asyncForm div").append('<textarea name="rawAnalysis"></textarea>');
  ...
  $("#asyncForm textarea[name=rawQuestion]").text($("input[name=rawQuestion]").val())
	$("#asyncForm textarea[name=question]").text($("input[name=question]").val().replace(new RegExp("<p><br></p>",'gi'), ""));
	$("#asyncForm textarea[name=analysis]").text($("input[name=analysis]").val().replace(new RegExp("<p><br></p>",'gi'), ""));
	...
	if(type==1||type==2){
		...
		for(var i=0;i<keyList.length;i++){
			if(i<=(keyList.length-1)){
				...
				answer=$(keyList[i]).find(".keyRight").children("input[name=answer"+(i+1)+"]").val().replace(new RegExp("<p><br></p>",'gi'), "");
...
function saveComb(){
	...
	$.ajax({
		...
		dataType: "json",
        contentType: "application/json",
		url: "/baseinfo/admin/edit_mix_question/?dataType=whole&addType=text",
		data: dataForm,
		success: function(msg){
			if(msg.success == true){
				resetPage();
