...
$(document).ready(function () {
  ...
  //击败百分比开启关闭
  $("body").on("click",".switch-Beatratio",function(e){
    e.stopPropagation();
    e.preventDefault();
...
//显示分享设置对话框
function showWeChat() {
  weChatModal.location.href = "/examadmin/admin/wechat_setting/";
  $('#weChatModal').modal({
    backdrop: "static",
    keyboard: false
  });
...
function showSelLogin(obj) {
  selLoginModal.location.href = "/examadmin/admin/skip_login_config";
  $('#loginModal').modal({
    backdrop: "static",
    keyboard: false
  });
...
function savePaperFn(url) {
  ...
  // phones : 字符串，以英文逗号分割，表示手机白名单
  // phoneExcelName : 手机白名单文件名
  // phoneExcelUrl : 手机白名单oss地址
