...
$(document).ready( function() {
  ...
  $("body").on("click", "#confirmCreatePaper", function () {
    if(checkPaperForm()){
      ...
      var perTimeRestrict =  $("#createPaperForm input[name=perTimeRestrict]").val();

      window.location.href = "/examadmin/admin/paper_manual_add?paper_name=" + escapeHTML(paperName) +
        "&paper_style=" + paperStyle + "&paperStyleName=" + paperStyleName + "&paper_type=" + paperType + "&add_style=select&perTimeRestrict=" + perTimeRestrict + "&addPaper=1";
    }
  });
  ...
  $("#grid-data").bootgrid().on("loaded.rs.jquery.bootgrid", function() {
    $("#grid-data").bootgrid().find(".previewPaper").on("click", function(e) {
      var paperId = $(this).attr("paperId");
      e.stopPropagation();
      e.preventDefault();
      window.open("/admin/paper_preview/?paper_info_id=" + paperId);
    })
  });
  //单个试卷编辑
  $("#grid-data").bootgrid().on("loaded.rs.jquery.bootgrid", function() {
...
function showSelType(type){
  if(selPaperStyleType=='single'){
    ...
    selTypeModal.location.href = "/admin/paper_list/paper_style_multi";
  }
  $('#typeModal').modal({
    backdrop:"static",
    keyboard:false
...
function paperEditFn(examId){
  ...
  return tempwindow.location="/admin/paper_update/#/"+examId+'?sourceFrom=paperList';
  $.ajax({
    type: "GET",
    cache : false,
    headers: { "cache-control": "no-cache" },
    dataType: "json",
    url: "/examadmin/admin/paper/is_using/"+examId,
    success: function(msg){
      if(msg.success== true){
        tempwindow.location="/examadmin/admin/paper/update/"+examId+'?sourceFrom=paperList';
...
function paperCopyFn(paperId) {
  $.ajax({
    type: "post",
    cache : false,
    url: "/examadmin/admin/paper_copy",
    headers: { "cache-control": "no-cache" },
    dataType: "json",
    data: {
      paperInfoId: paperId
