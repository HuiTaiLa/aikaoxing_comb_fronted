$('input[name=edit_paper_name]').val(getQueryString('paper_name'));
$('input[name=paperStyle]').val(getQueryString('paper_style'));
$('input[name=paperType]').val(getQueryString('paper_type'));
$('input[name=add_style]').val(getQueryString('add_style'));
$('input[name=perTimeRestrict]').val(getQueryString('perTimeRestrict'));
$('input[name=paperName]').val(getQueryString('paper_name'));
$('input[name=paper_style]').val(getQueryString('paper_style'));
$('input[name=paper_name]').val(getQueryString('paper_name'));
...
$(document).ready(function () {
  ...
  // if ( showIntroRights.isExamTest == 1 ){
  //   $('#introTopDialog').show();
  // }
  ...
  //   $('#introTopDialog').hide();
  //   $('#introTopDialog2').show();
  // });
  //
  // $('.closeIntroDialog').click(function () {
  //   $('#introTopDialog2').hide();
  // });

  // $('.prevIntroBtn').click(function (e) {
  //   e.stopPropagation();
  //   e.preventDefault();
  //   $('#introTopDialog2').hide();
  //   $('#introTopDialog').show();
  // });
  //
  // $('.closeIntroBtn').click(function (e) {
  //   e.stopPropagation();
  //   e.preventDefault();
  //   $('#introTopDialog').hide();
  // });
  ...
  $(".questionType_add").click(function (e) {
    ...
    if(paper_type == "1") {
      ...
      selTr = $(obj);
      if ($("input[name=classification]").val() != "" && $("input[name=add_style]").val() == "manual") {
  ...
  //选择分类
  $("body").on("click", "#selTypeLinkPaper", function(e) {
    e.stopPropagation();
    e.preventDefault();
    showSelTypePaper(this);
  });

  // 选择标签
  $("body").on("click", "#selLabelLinkPaper", function(e) {
    showSelLabel(this);
  });
  ...
  function showCheckedClassLabel(_this){
    ...
    if(class_ids==""){ //如果为空，文案变回请选择
      $("#selTypeLinkPaper").text('请选择');
    }
    if(label_ids==""){ //如果为空，文案变回请选择
      $("#selLabelLinkPaper").text('请选择');
    }
  }
  ...
  function actionPreviewWork(){
    if (checkForm_paper()) {
      setTimeout(function () {
        if (serializeForm_paper()) {
          $('#asyncForm_paper').ajaxSubmit({
            url : '/api/paper/paper_preview/',
            type : 'post',
            // dataType : 'json',
            // headers:
            //   {
    ...
    if (checkForm_paper()) {
      setTimeout(function () {
        if (serializeForm_paper()) {
          ...
          // $("#iframe-box").html('<iframe style="display: none;" name="asyncForm_paper" src="https://admin.kaoshixing.com/examadmin/admin/paper_download/"></iframe>');
  ...
  $('#createExam').click(function () {
    $('#savePaperModal').modal('hide');
    $("#savePaperBtn").removeClass('addPaperOnly');
    sessionStorage.setItem('ksxSaveData',0);
    // 继续创建考试
    window.location.href = getAdminUrlPrefix + "/examadmin/admin/exam_add?paper_info_id=" + paperInfoId;
  });
...
function paperTypeShowTemp() {
  ...
  if (paper_type == "1") {
    if (classification != 0) {
      title.text("手动录入题目");
      ajaxUrl = "/api/paper/paper_manual_add";
      return;
    } else {
      title.text("选题组卷");
  ...
  ajaxUrl = "/api/paper/paper_add_new";
}
  ...
  if (type == 1) {
    ...
    $(".group_main input[name=test_peer_time]").each(function (index, element) {
      if ($(this).attr("sort") == sortNum) {
        ...
        $(this).parents('.group_simple').find('.m-example').each(function (index,element) {
          var num = $(this).find('.member-question').length;
          $(this).find("input.member_second").val(Number($(obj).val() / num).toFixed(2));
        });
      }
    });
    totalTimeFn();
...
function saveDifficultFn(difficult, totelNum) {
  ...
  var text = "难度：简单：" + "<span class='diff1'>"+ list[0] + "</span>" + "；普通：" + "<span class='diff2'>"+ list[1] + "</span>" + "；困难：" + "<span class='diff3'>"+ list[2] + "</span>";
