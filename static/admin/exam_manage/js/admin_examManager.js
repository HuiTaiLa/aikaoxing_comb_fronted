...
$('#examTop').click(function(e){
  ...
  if($("#grid-data").bootgrid("getSelectedRows").length>3){
    ...
    $.ajax({
      ...
      dataType:'json',
      url: '/api/exam/getCurrentTopExam',
...
$(document).ready(function() {
  ...
  $("#grid-data").bootgrid({
    ...
    formatters: {
      "link": function(column, row)
      {
        return "<a href='#' data-growing-title='updateExam' class='icon-a_operate_edit updateExam' examId='"+row.id+"' data-toggle=\"tooltip\" data-placement=\"top\" data-container=\"body\" title='编辑'></a>"+
          ...
          // "<a href='#' data-growing-title='searchState' class='icon-a_operate_dspmsg searchState' data-toggle=\"tooltip\" data-placement=\"top\" data-container=\"body\" title='学员答题状态查询' examId='"+row.id+"'></a>";
      },
      "examName": function(column,row) {
        return "<a href='#' data-growing-title='searchState' class='expandExamName' data-toggle=\"tooltip\" data-placement=\"top\" data-container=\"body\" title=\""+row.examName+"\">"+ row.examName +"</a>";
      },
      "customSetProcess": function(column,row) {
        return row.setProcess=='是'&&row.processId!=0?"<a href='#' class='jumpProcess' processid='"+ row.processId +"' target='_blank' data-toggle=\"tooltip\" data-placement=\"top\" data-container=\"body\" title='关联任务链接'>"+ row.setProcess +"</a>":row.setProcess;
      }
    }
    ...
    $("#grid-data").bootgrid().find(".linkTest").on("click", function(e) {
      ...
      e.preventDefault();
      $.ajax({
        url: '/examadmin/admin/exam/exam_can_edit/'+examId,
        type: 'get',
        cache: false,
        dataType: 'json',
        success: function (msg) {
          if (msg.success) {
            ...
            $.ajax({
              ...
              headers: { "cache-control": "no-cache" },
              dataType: "json",
              url: "/examadmin/admin/exam/link_info/?examId=" + examId,
              success: function(msg) {
                //该地址和modal框里的地址不一样
    ...
    //关联自定义任务链接
    $("#grid-data").bootgrid().find(".jumpProcess").on("click", function(e) {
      window.open(window.getStaticUrlPrefix + "/admin/customprocess/#/create/"+ $(this).attr("processid"));
      e.stopPropagation();
      e.preventDefault();
    });
    //考试链接
    ...
    $("#grid-data").bootgrid().find(".linkAnalysis").on("click", function(e) {
      ...
      e.preventDefault();
      var _this = $(this);
      window.open(encodeURI(getAdminUrlPrefix+"/analysis/ae/results/analysis/exam?examId="+_this.attr("examId")+"&examName="+encodeURI(_this.attr("examName"))));
    });
  });
        ...
        buttons: [{
          ...
          action: function(dialogItself) {
            ...
            $.ajax({
              ...
              headers: { "cache-control": "no-cache" },
              dataType: "json",
              url: '/examadmin/admin/results/import/?examInfoId=' + dataForm + "&confirmExaminees=0" + "&batch=1",
              success: function(msg) {
                $('#exportModal').modal({
                  backdrop: "static",
                  keyboard: false
          ...
          action: function(dialogItself) {
            ...
            $.ajax({
              ...
              headers: { "cache-control": "no-cache" },
              dataType: "json",
              url: '/examadmin/admin/results/import/?examInfoId=' + dataForm + "&confirmExaminees=1" + "&batch=1",
              success: function(msg) {
                $('#exportModal').modal({
                  backdrop: "static",
                  keyboard: false
  ...
  if (document.URL.match("exam_list") == "exam_list") {
    //显示选择分类对话框
    function showSelType(obj) {
      selTypeModal.location.href = "/admin/exam_list/exam_sel_style_multi";
      $('#typeModal').modal({
        backdrop: "static",
        keyboard: false
      });
    ...
    //显示选择分类对话框
    function showSelType(obj) {
      selTypeModal.location.href = "/admin/exam_list/exam_sel_style";
      $('#typeModal').modal({
        backdrop: "static",
        keyboard: false
      });
  ...
  function examEditFn(examId) {
    return window.open("/admin/exam_update/#/" + examId);
    $.ajax({
      url: '/examadmin/admin/exam/exam_can_edit/'+examId,
      type: 'get',
      cache: false,
      ...
      success: function(msg) {
        if (msg.success != false)
          window.open("/examadmin/admin/exam/update/" + examId);
  ...
  function getExamUrl(examId, examName) {
    ...
    $.ajax({
      ...
      headers: { "cache-control": "no-cache" },
      dataType: "json",
      url: "/api/exam/link_info/?examId=" + examId + "&account=" + user.account + "&companyId=" + user.companyId,
      success: function(msg) {
        if(msg.success){
          if(msg.bizContent.setProcess==1){
            $('#cannotLinkModal').modal("show");
          }else {
            //二维码链接图片内容更新
            var activity_type="考试";
            var title=msg.bizContent.examName;
            var start_time=msg.bizContent.examStartTime;
            var end_time=msg.bizContent.examEndTime;
            var exam_time=msg.bizContent.examTime!=-1?msg.bizContent.examTime+'分钟':'';
            ...
            showSelOk(examId, msg.bizContent.examUrl, msg.bizContent.examPwd, url, 'exam', msg.bizContent.isSkipLogin,"考试")
          }
        }else {
  ...
  function examDelFn(examId, much) {
    BootstrapDialog.show({
      title: "确定删除选中的记录?",
      message: "删除考试会同步删除考试中的成绩和答题，是否继续？<br /> 以下情况将导致本场考试无法删除：<br /> -本场考试当前有人正在答题<br />",
          ...
          action: function(dialogItself)
          {
            ...
            $.ajax({
              ...
              success: function(msg) {
                if (msg.success != true) {
                  if (msg.code == 33061) {
                    createDialog("有学员正在参加该考试");
                  } else if (msg.code == 33076) {
                    createDialog("操作失败，此考试已关联自定义任务，请前往自定义任务中取消关联后再试");
                  } else {
  ...
  function examStatusFn(examId, type) {
    if (confirm("确定更改所选择的考试状态？")) {
      $.ajax({
        ...
        headers: { "cache-control": "no-cache" },
        dataType: "json",
        url: "/examadmin/admin/exam/update_status/" + examId + "/" + type,
        success: function(msg) {
          if (msg.success == true) {
...
$("#searchStateModal .btn-search").click(function(){
  ...
  $.ajax({
    ...
    headers: { "cache-control": "no-cache" },
    dataType: "json",
    url: "/examadmin/admin/query_examinee_qualifier",
    data: {
      examInfoId: examId,
      account: account
    },
    success: function(msg) {
      var code = msg.code?msg.code:0,
