$(document).ready(function () {
  ...
  //导出超限的条数
  const exportOverFlowResult=30000; //全部成绩
  const exportOverFlowHighScore=30000; //最高分成绩
  const exportOverFlowAnswer=30000; //答案
  const exportOverFlowPaper=3000; //试卷
  ...
  $("body").on("click", "#forceSubmitBtn", function (e) {
    ...
    BootstrapDialog.show({
      ...
      title: '批量交卷',
      message: '批量交卷会立即结束学员答题，并进行交卷和判分处理，是否继续?\n注：交卷结束后可进入消息中心查看交卷情况',
      buttons: [{
        label: '取消',
        ...
        action: function(dialog) {
          ...
          $.ajax({
            ...
            dataType: "json",
            url: '/examadmin/exam/batch/force_submit',
  ...
  $("#resetSaveBtn").click(function (e) {
    ...
    $.ajax({
      ...
      headers: { "cache-control": "no-cache" },
      dataType: "json",
      url: '/examadmin/admin/exam/setDelayTime',
      data: dataForm,
      success: function (msg) {
        if(msg.success){
  ...
  $("body").on("click", "#searchIcon", function (e) {
    ...
    $("input[name=is_commit_search]").val("-1");
    $("input[name=force_commit_search]").val("-1");
    ...
    $("select[name=is_commit]").val("-1");
    $("select[name=force_commit]").val("-1");
  });
  ...
  $("body").on("click", "#searchBtn", function (e) {
    ...
    $("input[name=is_commit_search]").val($("select[name=is_commit]").find("option:selected").val());
    $("input[name=force_commit_search]").val($("select[name=force_commit]").find("option:selected").val());
    $("#grid-data").bootgrid("reload");
  });
  //搜索回车事件
  ...
  $(".export-btn").click(function(){
    ...
    var dataForm = "examInfoId=" + exam_info_id + "&userName=" + user_name + "&depId=" + dep_id +
      ...
      "&isCommit="+is_commit+"&forceCommit="+force_commit;
    ...
    if($(".all-result").hasClass("export-style-active")){
      ...
      var total=parseInt($("#byTest").find(".infos").text().match(/\d+/g)[0]);
      if(total>exportOverFlowResult && selectList.length==0){
        $("#exportAmountOverflow .export_type").text("成绩");
        $("#exportAmountOverflow .amount").text(exportOverFlowResult);
        $('#exportAmountOverflow').modal({
          backdrop: "static",
          keyboard: false
        });
        return;
    ...
    if($(".high-score").hasClass("export-style-active")){
      ...
      $.ajax({
        ...
        success: function(msg) {
          if(msg.bizContent!=null){
            if(msg.bizContent.exportStatus == "false" && msg.bizContent.code == "61523") { //导出条数过多
              $("#exportAmountOverflow .export_type").text("成绩");
              $("#exportAmountOverflow .amount").text(exportOverFlowHighScore);
              $('#exportAmountOverflow').modal({
                backdrop: "static",
                keyboard: false
              });
  ...
  $(".exportBtn_answer").click(function (e) {
    ...
    if(dataForm != ""){
      ...
      if (selectList.length == 0){
        dialogTip = "请确认是否导出全部学员的答案？";
      }else {
        dialogTip = "请确认是否导出当前选中学员的答案？";
        dataForm += "&examResultIds=" + selectList.join(",");
      }
      BootstrapDialog.show({
        title: "导出学员答案",
        ...
        buttons: [
          ...
          },
          {
            ...
            action: function(dialogItself) {
              dialogItself.close();
              var total=parseInt($("#byTest").find(".infos").text().match(/\d+/g)[0]);
              if(total>exportOverFlowAnswer && selectList.length == 0){ //导出条数过多
                $("#exportAmountOverflow .export_type").text("答案");
                $("#exportAmountOverflow .amount").text(exportOverFlowAnswer);
                $('#exportAmountOverflow').modal({
                  backdrop: "static",
                  keyboard: false
                });
              ...
              $.ajax({
                ...
                headers: { "cache-control": "no-cache" },
                dataType: "json",
                url: '/examadmin/admin/answers/download/?' + dataForm,
                success: function(msg) {
  ...
  $(".exportBtn_paper").click(function (e) {
    ...
    var dataForm = "examInfoId=" + exam_info_id + "&userName=" + user_name + "&depId=" + dep_id +
      ...
      + "&examVerifyStatus=" +verify_status+"&isCommit="+is_commit+"&forceCommit="+force_commit;
    ...
    if(dataForm!=""){
      ...
      var selectList = $("#grid-data").bootgrid("getSelectedRows").reverse();//解决导出倒序问题;
      if (selectList.length == 0){
        dialogTip = "请确认是否导出全部学员的答卷？";
      }else {
        dialogTip = "请确认是否导出当前选中学员的答卷？";
        dataForm += "&examResultIds=" + selectList.join(",");
      }
      BootstrapDialog.show({
        title: "导出学员答卷",
        ...
        buttons: [
          ...
          },
          {
            ...
            action: function(dialogItself) {
              dialogItself.close();
              var total=parseInt($("#byTest").find(".infos").text().match(/\d+/g)[0]);
              if(total>exportOverFlowPaper && selectList.length == 0){
                $("#exportAmountOverflow .export_type").text("答卷");
                $("#exportAmountOverflow .amount").text(exportOverFlowPaper);
                $('#exportAmountOverflow').modal({
                  backdrop: "static",
                  keyboard: false
                });
              ...
              $.ajax({
                ...
                headers: { "cache-control": "no-cache" },
                dataType: "json",
                url: '/examadmin/exam/result/batch_export_pdf/?' + dataForm,
                success: function(msg) {
                  if(!msg.success){
                    alert(msg.desc);
                    return false;
          ...
          },
          {
            ...
            action: function(dialogItself) {
              ...
              $.ajax({
                ...
                headers: { "cache-control": "no-cache" },
                dataType: "json",
                url: '/examadmin/admin/results/export_absence/?examInfoId=' + exam_info_id,
                success: function(msg) {
...
function showSelDept(obj,userId){
  updateDeptModal.location.href = "/baseinfo/admin/tree/user_sel_dep/"+userId;
  $('#deptModal').modal({
    backdrop:"static",
    keyboard:false
  });
...
function showSelDep(obj) {
  // selDepModal.location.href = "/baseinfo/admin/tree/user_sel_dep/00000";
  selDepModal.location.href = "/admin/exam_add/user_sel_dep_multi";
  $('#depModal').modal({
    backdrop: "static",
    keyboard: false
  });
...
function recoverExam(id,exam_info_id) {
  BootstrapDialog.show({
    cssClass: 'batch-submit',
    title: "恢复考试",
    message: "恢复考试能够将学员已经提交的试卷恢复到提交前的状态，学员能够接着提交前的试卷继续答题，正在考试的" +
    "学员试卷将无法被退回。请确认是否将选中的学员进行恢复考试操作。",
    buttons: [{
      cssClass: 'batch-submit-no',
      label: "取消",
      action: function (dialogItself) {
      ...
      action: function (dialogItself) {
        ...
        $.ajax({
          ...
          headers: { "cache-control": "no-cache" },
          dataType: "json",
          url: "/examadmin/exam/back_exam_results/",
          data: dataForm,
          success: function (msg) {
            if(msg.success) {
              alert("操作成功！");
