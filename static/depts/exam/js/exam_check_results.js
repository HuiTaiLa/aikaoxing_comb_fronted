$(function () {
  ...
  //显示答题卡
  $("body").on("click", "#numberCardBtn", function (e) {
    e.stopPropagation();
    e.preventDefault();
  ...
  //点击答题卡跳转至对应题,使用了锚点跳转
  //位置调整90（因为有顶栏），并关闭答题卡
    $("body").on("click","#numberCardModal .modal-body .box", function (e) {
    //如果是错题本，编辑状态
  ...
  //*******************************手动判分**********************************
  //更改正确错误状态
  $("body").on("click",".operation-check .icon",function (e) {
    e.preventDefault();
    var _this = $(this);
  ...
  //上一人下一人
  $("body").on("click","#prevOneBtn, #nextOneBtn",function () {
    // 跳转地址
    var jumpUrl = '/examadmin/exam/manmade_exam_check';
    var _this = $(this);
  ...
  //保存
  $("body").on("click","#saveCheckResultsBtn",function () {
    if(checkResultsForm() && checkAllComments()){
      ...
      $.ajax({
        ...
        headers: { "cache-control": "no-cache" },
        dataType: "json",
        url: "/examadmin/exam/manmade_exam_ending",
        data: dataJson,
        success: function(msg){
          if(msg.success){
            //判断是否生成证书
...
$("body").on("click",".filterOnly .current_filter",function(e){
  e.stopPropagation();
  e.preventDefault();
  $(".filterOnly .filter_opts").show();
})
$("body").on("click",".filter_opt",function(e){
  e.stopPropagation();
  e.preventDefault();
...
//新答题卡-收起
$("body").on("click","#answercardFoldBtn",function(){
...
//新答题卡-展开
$("body").on("click","#answercardOpenBtn",function(){
