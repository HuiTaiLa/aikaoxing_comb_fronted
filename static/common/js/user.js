$(function () {
  ...
  $("input[name=picture]").change(function() {
    ...
    $('#characterForm').ajaxSubmit({
      ...
      success:function (msg){
        if(msg.success){
          $(".character-img").attr("src",msg.bizContent.url);
  ...
  $("body").on("click", ".modal-user-info .btn-wechat.binded", function (e) {
    ...
    $.ajax({
      ...
      dataType: "json",
      url: "/login/account/user/unbind_wechat",
      success: function (msg) {
        if(msg.success){
  ...
  $("#checkEmailBtn").click(function (e) {
    ...
    if(email==''){
      ...
      $.ajax({
        ...
        headers: { "cache-control": "no-cache" },
        dataType: "json",
        url: "/setting/admin/user/verify_emails/" + userId,
        success: function(msg) {
          if (msg.success) {
            alert("验证邮件已发送");
          }else {
  ...
  $("#savePasswordBtn").click(function(e) {
    ...
    var dataForm = "oldPassword=" + oldPassword +
